def otimizacao_PS_SG_RT():
    ...
    Q_bat = pl.LpVariable.dicts('Q_bat', ((tempo) for tempo in dados_entrada.index), lowBound=-0.99, upBound=0.99, cat='Continuous')
    
    P_bat = pl.LpVariable.dicts('P_bat', ((tempo) for tempo in dados_entrada.index), lowBound=-1.0, upBound=1.0, cat='Continuous')
    ...
    delta_1 = pl.LpVariable.dicts('delta_1',((tempo) for tempo in dados_entrada.index), cat='Binary')
    
    delta_2 = pl.LpVariable.dicts('delta_2',((tempo) for tempo in dados_entrada.index), cat='Binary')
    
    delta_3 = pl.LpVariable.dicts('delta_3',((tempo) for tempo in dados_entrada.index), cat='Binary')
    
    delta_4 = pl.LpVariable.dicts('delta_4',((tempo) for tempo in dados_entrada.index), cat='Binary')
    ...
    prob += pl.lpSum([(0.2*(P_rede[i]*ts*dados_entrada.loc[i,'custo_energia']*(1/80))) + 
                      (0.1*((P_dc[i]+P_ch[i])*ts*d*(1/22.5))) +
                      (0.5*(multa[i]*1/50)) +
                      (0.05*((t1_soc[i]+t2_soc[i])*(1/0.6))) +
                      (0.1*((t1[i]+t2[i])*(1/60))) +
                      (0.05*(t1_reg[i]+t2_reg[i])) for i in dados_entrada.index])
    ...
    prob += pl.lpSum([(0.3*(P_rede[i]*ts*dados_entrada.loc[i,'custo_energia']*(1/80))) + 
                      (0.1*((P_dc[i]+P_ch[i])*ts*d*(1/22.5))) +
                      (0.2*(multa[i]*1/50)) +
                      (0.03*(t1_soc[i]+t2_soc[i])) +
                      (0.2*((t1[i]+t2[i])*(1/60))) +
                      (0.07*(t1_reg[i]+t2_reg[i])) for i in dados_entrada.index])
    ...
    for i in dados_entrada.index:
        ...
        prob += V_PCC[i] == (-(1/K)*(dados_entrada.loc[i, 'potencia_reativa_prevista'] - (Q_bat[i]*250))) + 0.9955
        
        prob += t1_reg[i] - t2_reg[i] == (V_PCC_ref - V_PCC[i])
 
     #------------- CURVA CAPABILIDADE INVERSOR LINEARIZADA - 1 -------------------#
        '''
        ...
        '''
        prob += Q_bat[i] <= (-0.990*P_bat[i] + 1)/(0.141)
        prob += Q_bat[i] <= (-0.891*P_bat[i] + 1)/(0.454)
        prob += Q_bat[i] <= (-0.792*P_bat[i] + 1)/(0.610)
        prob += Q_bat[i] <= (-0.693*P_bat[i] + 1)/(0.720)
        prob += Q_bat[i] <= (-0.594*P_bat[i] + 1)/(0.804)
        prob += Q_bat[i] <= (-0.495*P_bat[i] + 1)/(0.868)
        prob += Q_bat[i] <= (-0.396*P_bat[i] + 1)/(0.918)
        prob += Q_bat[i] <= (-0.297*P_bat[i] + 1)/(0.954)
        prob += Q_bat[i] <= (-0.198*P_bat[i] + 1)/(0.980)
        prob += Q_bat[i] <= (-0.099*P_bat[i] + 1)/(0.995)
        prob += Q_bat[i] <= (0*P_bat[i] + 1)/(1)
        prob += Q_bat[i] <= (0.099*P_bat[i] + 1)/(0.995)
        prob += Q_bat[i] <= (0.198*P_bat[i] + 1)/(0.980)
        prob += Q_bat[i] <= (0.297*P_bat[i] + 1)/(0.954)
        ...
        if i == 0:
            ...
            # P_dc[i] >= P_SAE_DC_ANT:
            prob += P_dc[i] - P_SAE_DC_ANT >= L*(1-delta_1[i])
            prob += P_dc[i] - P_SAE_DC_ANT <= (U+epsilion)*delta_1[i] - epsilion
        
            prob += P_dc[i] - P_SAE_DC_ANT -50 <= U*(1-delta_1[i])
        
            # P_dc[i] < P_SAE_DC_ANT:
            prob += P_dc[i] - P_SAE_DC_ANT <= (U+epsilion)*(1-delta_2[i]) - epsilion
            prob += P_dc[i] - P_SAE_DC_ANT >= L*delta_2[i]
            
            prob += P_SAE_DC_ANT - P_dc[i] - 50 <= U*(1-delta_2[i])
            
        
            # P_ch[i] >= P_SAE_CH_ANT:
            prob += P_ch[i] - P_SAE_CH_ANT >= L*(1-delta_3[i])
            prob += P_ch[i] - P_SAE_CH_ANT <= (U+epsilion)*delta_3[i] - epsilion
        
            prob += P_ch[i] - P_SAE_CH_ANT -50 <= U*(1-delta_3[i])
        
            # P_ch[i] < P_SAE_CH_ANT:
            prob += P_ch[i] - P_SAE_CH_ANT <= (U+epsilion)*(1-delta_4[i]) - epsilion
            prob += P_ch[i] - P_SAE_CH_ANT >= L*delta_4[i]
            
            prob += P_SAE_CH_ANT - P_ch[i] - 50 <= U*(1-delta_4[i])
        ...
        if i >=1:
            prob += var_dc[i] == P_dc[i] - P_dc[i-1]
            prob += var_ch[i] == P_ch[i] - P_ch[i-1]   
            
            prob += x1[i] - x2[i] == var_dc[i]
            prob += y1[i] - y2[i] == var_ch[i]
